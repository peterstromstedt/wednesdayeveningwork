...
namespace wednesdayeveningwork
{
    ...
    {
        public static int SideTimesBase(int oneSide, int theBase)
        {
            return oneSide * theBase / 2;
        }

        public static double ThreeSides(double firstSide, double secondSide, double thirdSide) 
        {
            double sum = (firstSide + secondSide + thirdSide) / 2;
            return Math.Sqrt(sum * (sum - firstSide) * (sum - secondSide) * (sum - thirdSide));
        }

        public static double TwoSidesAndAngle(double firstSide, double secondSide, double angle)
        {            
            double angleRadiance = angle * Math.PI / 180;
            return 0.5 * firstSide * secondSide * Math.Sin(angleRadiance);
        }
    }
